...
class SecondScreen(Screen):
    def __init__(self, **kwargs): #self o day la screen nen phai add them layout
        ...
        self.time=[]
    def on_enter(self):
        Clock.schedule_interval(self.flash, 0.8) #sau khi an play thi moi bat dau 
    ...
    def flash(self,dt):
        ...
        self.count =self.count+random.randint(2,5)
        self.button.append(x)
        if self.count >=9:  #>=4 thi loop se ngat
    ...
    def on_touch_down(self,touch):
        for i in range (1,10):
            if self.ids['button'+str(i)].collide_point(*touch.pos):
                timeout=0
                self.buttons.append('button'+str(i))
                timeout+=time.perf_counter()
                self.time.append(timeout)
                print(self.buttons)
                print(self.time[-1]-self.time[0])
        Clock.schedule_once(self.result, 5) #problem is here
    ...
    def result(self,dt):
        if self.button!=self.buttons or (self.time[-1]-self.time[0])>5 :
